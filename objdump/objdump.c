...
** Last update Mon Feb 27 10:18:56 2017 Nicolas Guillon
*/
...
int		headerFile(void *buff, struct stat s, char *file, int ac)
{
  ...
  if (ver == 2)
	...
	return (0);
      return (printSecs(buff, file, ac, s));
    }
  else if (ver == 1)
    {
      if (!checkSize32(buff, file, s))
	return (0);
      return (printSecs32(buff, file, ac, s));
    }
  else
    return (-1);
}
...
int		statMapFile(int fd, char *file, int ac)
{
  ...
  fstat(fd, &s);
  if ((buff = mmap(NULL, s.st_size, PROT_READ, MAP_PRIVATE, fd, 0)) == (void *)-1)
    {
